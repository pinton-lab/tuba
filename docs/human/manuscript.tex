...
volume is fetched from Zenodo by \texttt{tuba.data.fetch\_human}; the
MNI152 ICBM 2009a non-linear symmetric template and its parcellations
are fetched lazily through \texttt{tuba.atlases.mni152} (nilearn-driven,
with the catalogue under \texttt{tuba.atlases.mni152.PARCELLATIONS});
the processing pipeline lives at \texttt{src/tuba/species/human.py} with
intermediate caches in \texttt{\$TUBA\_HUMAN\_REG\_DIR}.
\end{abstract}
...
in the user's nilearn cache directory (typically
\texttt{\textasciitilde/nilearn\_data/icbm152\_2009/}); both are fetched
through \texttt{tuba.atlases.mni152} via
\texttt{nilearn.datasets.fetch\_icbm152\_2009}.
...
\texttt{tuba.species.human} (the \texttt{halle\_affine\_to\_ras} helper);
downstream code that writes or reads the Halle volume {\em must} go
through this module rather than trusting the NRRD header field.
...
angles (\S5 of the mouse report~\cite{maga_manuscript}; angles $(\alpha_x, \alpha_y,
...
(\texttt{neuromod\_parameters/\allowbreak registration/\allowbreak
register\_orientation\_first.py}; not ported into TUBA) exists, using
a five-point Umeyama fit on nasion, inion, the two pre-auriculars, and
the vertex. It is not invoked by the production pipeline because ANTs
SyN handles the same residual orientation drift directly, and the
landmark auto-detection produced noisier results than the SyN affine
init in side-by-side QC. The script is retained in the legacy
\texttt{neuromod\_parameters} layout for reproducibility of historical
figures but is not on the TUBA code path.
...
The legacy
\texttt{neuromod\_parameters/\allowbreak registration/\allowbreak
extract\_halle\_surface.py} (not ported into TUBA) takes the
...
\texttt{tuba.species.human.\allowbreak place\_on\_skull} (resolved via
\texttt{OUTER\_ENVELOPE\_FALLBACK}) in case the ray-cast cache below
has not been generated, and as a registration-target proxy for the
legacy landmark-based scripts.

\subsection{Ray-cast outer table (the production path)}
\label{sec:surface_raycast}
\texttt{tuba.species.human.\allowbreak extract\_outer\_bone\_surface}
solves the inner-table problem by ray-casting outward from the
bone-mass centroid.
...
The forward + inverse transforms are persisted under
\texttt{\$TUBA\_HUMAN\_REG\_DIR} and indexed by stable text
lists:
...
\item {\em Trust the data, not the NRRD header.} The Halle release tags
  ...
  the species module (\texttt{tuba.species.human.\allowbreak
  halle\_affine\_to\_ras}); downstream code must not re-derive the
  storage convention from the header.
...
Accessed 2026-04 via \texttt{tuba.data.fetch\_human}.
\bibitem{fonov2009}
