...
29 animals, histologically-defined cortical labels). The pillar reuses
...
University Institute of Imaging Science and distributed CC~BY via
NITRC (RRID:SCR\_015542). VALiDATe29 is multi-channel: anatomical
templates (proton-density, $T_1$, $T_2^*$), diffusion templates (FA,
MD), ex-vivo templates, and --- critically for this project ---
\textbf{histologically-defined cortical labels} co-registered from
Nissl/myelin sections, plus tractography-defined white-matter labels.
It supersedes the earlier single-subject squirrel-monkey MRI
atlas~\cite{gao-2014}.

\paragraph{Layout tolerance.}
The exact filenames inside \texttt{VALiDATe29.zip} drift across releases
1.0/1.1/1.2 (several template channels, more than one label volume), so
the binding \texttt{tuba.atlases.\allowbreak validate29.\allowbreak
VALiDATe29} does not hard-code them: it \emph{resolves} the template,
the cortical-label volume, and the label table by globbing the staged
directory with keyword heuristics, and raises with the candidate list
when resolution is ambiguous rather than guessing. Every path is
overridable by constructor argument or environment variable
(\texttt{VALIDATE29\_DEST}, \texttt{VALIDATE29\_URL}).

\paragraph{Target structures.}
The study targets the \emph{hand} representations in S1 areas 3b and 1
and in M1 (area 4); S1 and M1 are only $\sim$2--3\,mm apart in the
squirrel monkey, which is precisely why the downstream two-foci
feasibility question (\S\ref{sec:scope-out}) is non-trivial. Because
the atlas label spellings vary across releases,
\texttt{validate29.\allowbreak resolve\_label} matches \emph{stable
area-number substrings} (\texttt{'area 3b'}, \texttt{'area 1'},
\texttt{'area 4'}) rather than a full canonical name; the exact
spelling is confirmed against the staged label table
(\S\ref{sec:open}). Localising the hand sub-representation \emph{within}
each area additionally requires VALiDATe29's stereotaxic frame (a
mediolateral/AP prior on the homunculus), applied once the atlas
coordinate frame is confirmed; until then \texttt{target\_world\_mm}
returns the whole-area centroid and says so.
...
nearest-neighbour for the labels). The brain mask is derived from the
label volume (\texttt{label $> 0$}), not a template threshold ---
the same lesson the rat and mouse pillars record.

\paragraph{Target export + QC (deliverable 2 outputs).}
\texttt{export\_targets()} writes S1-3b, S1-area-1, and M1
hand-representation coordinates in the subject simulation frame to
...
\texttt{saimiri\_cavity\_to\_validate29}. The exported coordinates and
the QC figure are produced by \texttt{build()} once the scan + atlas
are staged; their values are \PENDING{}.
...
Because no scan is staged, verification is by offline unit tests
(\texttt{tests/test\_saimiri.py}, 11 tests, no network / no antspyx /
no data), covering the logic that must hold regardless:
...
The pillar's machinery is complete and tested; the remaining items all
require the staged scan (and, for quantitative acoustics, an
HU-calibrated colony CT):
\begin{enumerate}[leftmargin=*]
\setlength\itemsep{0pt}
  \item Stage the museum microCT (\texttt{python -m
        tuba.data.fetch\_saimiri}) and the VALiDATe29 atlas
        (\texttt{python -m tuba.data.fetch\_validate29}), then run
        \texttt{python -m tuba.species.saimiri build}.
  \item Calibrate the \texttt{PROVISIONAL} constants (orientation
        ...
        registration QC figures.
  \item Confirm the VALiDATe29 cortical-label spellings against the
        staged LUT and pin them (\S\ref{sec:atlas}); add the
        stereotaxic hand-sub-representation prior.
