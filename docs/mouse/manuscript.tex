...
staged with \texttt{templates/fetch\_maga\_skull.py}; processing
modules live alongside the DigiMouse pipeline in
\texttt{registration/}.
\end{abstract}
...
and PET. The pipeline currently in
\texttt{registration/}\allowbreak\texttt{mouse\_ct\_constants.py}
resamples this to
...
\small
\input{tables/source_compare.tex}
\end{table}
...
\texttt{registration/}\allowbreak\texttt{allen\_template\_10um.nii.gz}
and \texttt{allen\_annotation\_10um.nii.gz} (1140${\times}$1320${\times}$800
voxels, $\sim$11.4${\times}$13.2${\times}$8.0\,mm$^3$, RAS-affined) via
\texttt{templates/fetch\_allen\_ccfv3.py}, and use the 25\,$\mu$m
...
\texttt{registration/maga\_skull\_constants.py} (\texttt{SLICE\_FLIP =
True}) so that the RAS-aligned NIfTI satisfies the convention
...
\texttt{maga\_skull\_constants.py} so that the output volume satisfies
...
(see \texttt{downsample\_to} in
\texttt{registration/}\allowbreak\texttt{maga\_skull\_constants.py}).
The default 200\,$\mu$m
...
\texttt{maga\_skull\_constants.py}). In the same step the cortical-bone
...
(\texttt{registration/}\allowbreak\texttt{extract\_mouse\_cavity.py})
contains $\sim$300 lines and must solve several problems specific to
...
\texttt{registration/}\allowbreak\texttt{extract\_maga\_cavity.py}.
...
pipeline
(\texttt{registration/}\allowbreak\texttt{register\_cavity\_to\_allen.py})
with Maga-specific inputs and outputs. The Maga-version script
(\texttt{registration/}\allowbreak\texttt{register\_maga\_}\allowbreak\texttt{cavity\_to\_allen.py})
reads
...
\texttt{maga\_skull\_constants.py} been wrong, or the rigid
...
in \texttt{registration/mouse\_skull\_slab\_3d.py}: voxels below the
...
\texttt{registration/}\allowbreak\texttt{maga\_skull\_}\allowbreak\texttt{slab\_3d.py}.
The Maga slab loader exposes the same
\texttt{load\_maga\_skull\_slab\_3d(apex\_voxel\_mm, beam\_3d,
slab\_size\_m, dx\_target\_m)} signature as the DigiMouse
\texttt{load\_mouse\_skull\_slab\_3d} so the FDTD/angular-spectrum
driver in \texttt{run\_mouse\_cc.py} is drop-in compatible (only the
skull import and the placement helper differ between the two
pipelines).
...
The forward simulation in
\texttt{run\_mouse\_cc.py} is now skull-agnostic: a
\texttt{MOUSE\_CC\_SKULL}~$\in\{$\texttt{digimouse},
\texttt{maga}$\}$ environment variable selects between the
\texttt{mouse\_skull\_slab\_3d} / \texttt{mouse\_placement} pair
(DigiMouse) and the
\texttt{maga\_skull\_slab\_3d} / \texttt{maga\_placement} pair
(Maga 4K), without otherwise changing the bowl geometry, drive level,
target convention, FDTD/angular-spectrum grid, attenuation model, or
...
extracted from both runs by
\texttt{validation\_report\_maga/make\_focal\_comparison.py}.
Fig.~\ref{fig:focal-comparison} shows the corresponding skull
cross-sections, $|p|$-fields (peak-normalised in dB), and on-axis /
lateral pressure profiles through the focal peak.
...
intensity volume from the slab grid back into the Allen frame. This
is implemented in
\texttt{validation\_report\_maga/}\allowbreak
\texttt{make\_intensity\_on\_atlas.py}: each voxel of the Maga 25\,$\mu$m
...
\item Acoustic-property mapping: piecewise-linear intensity ---
      ...
      the previous step.
\item Placement helper analogous to \texttt{maga\_placement.py}:
      extract the target centroid from the warped annotation,
      project to the closest dorsal outer-bone surface point, apply
      scalp clearance / apex-to-target geometry and an optional
      sagittal-plane tilt.
\item Forward simulation: the same
      \texttt{run\_mouse\_cc.py}-style driver with the placement
      helper / slab loader pair swapped for the NHP versions, and the
      drive calibration (free-field MI target $\to$ post-hoc
      $p_0$ rescaling) preserved unchanged.
